...
// The following functions define how to convert between view types. It is a
// sparse $n \times n$ matrix, where $n$ is the number of differrent view
// types. To convert from a view with `ViewType` `T` to a view with `ViewType`
// `R`, lookup the conversion function stored at the `VIEW_CONVERSIONS[T][R]`
// index. Most conversions are not yet defined---we can define them as we need.
// This matrix should stay private, and only be exposed through the
// `convertView` helper.
